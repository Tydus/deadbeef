...
gtk_enum_sound_callback (const char *name, const char *desc, void *userdata) {
    ...
    gtk_combo_box_append_text (GTK_COMBO_BOX (combobox), desc);
...
preferences_fill_soundcards (void) {
    ...
    gtk_combo_box_append_text (GTK_COMBO_BOX (combobox), _("Default Audio Device"));
                                        ...
                                        gpointer         user_data)
{
    ...
    for (int i = 0; out_plugs[i]; i++) {
        gtk_combo_box_append_text (GTK_COMBO_BOX (combobox), out_plugs[i]->plugin.name);
...
on_gui_plugin_changed                  (GtkComboBox     *combobox,
                                        gpointer         user_data)
{
    gchar *txt = gtk_combo_box_get_active_text (GTK_COMBO_BOX (combobox));
    if (txt) {
